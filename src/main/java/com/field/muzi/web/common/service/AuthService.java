...
public class AuthService {
    ...
    public MemberEntity signupAdmin(SignupRequest req) {
        MemberEntity member = MemberEntity.adminCreate(req.getMemberId(), passwordEncoder.encode(req.getMemberPwd()), req.getRole());
        if (memberRepository.existsByMemberId(member.getMemberId())) {
            throw new CInvalidValueException.CAlreadySignedupException();
        }
        if(req.getRole().equals("ADMIN")) {
            member.getMemberInfo().updateName("관리자");
        }else {
            member.getMemberInfo().updateName("부운영자");
        }
