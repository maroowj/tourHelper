...
public class BannerService {
    ...
    public void bannerInsert(BannerSaveRequest request) throws Exception {
        ...
        FileManagerEntity fileManager;

        if(request.getBannerImage() != null && !request.getBannerImage().isEmpty()) {
            bannerList.add(request.getBannerImage());
            fileManager = fileManagerRepository.save(fileHandler.parse(bannerList, "banner_img").get(0));
        }else{
            fileManager = null;
        }
    ...
    /** 선택한 배너 삭제 **/
    @Transactional
    public void deleteBanner(List<String > bannerSeqList) {
        for(String bannerSeq:bannerSeqList) {
